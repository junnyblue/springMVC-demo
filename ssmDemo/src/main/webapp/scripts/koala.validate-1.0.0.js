...
(function(factory) {
	...
	var reg_date = new RegExp("^(?:(?:([0-9]{4}(-|\/)(?:(?:0?[1,3-9]|1[0-2])(-|\/)(?:29|30)|((?:0?[13578]|1[02])(-|\/)31)))|([0-9]{4}(-|\/)"
			+ "(?:0?[1-9]|1[0-2])(-|\/)(?:0?[1-9]|1\\d|2[0-8]))|(((?:(\\d\\d(?:0[48]|[2468][048]|[13579][26]))|(?:"
	...
	var validation = $V = $.validation = {
		...
		validators : {
			...
			equalTo : {
				...
				getMessageParameters : function() {
					var name = $(this).getLabel();
					var target = $(this).attr('equals') || $(this).attr("equalTo");
					var value = koala.isExists($(target)) ? $(target).getLabel() : target;
					return {
						name : name,
						value : value
	...
	$.validate = $.validationSetup = function(config) {
		...
		if (validator) {
			for (key in validator) {
				var val = $.validation.validators[key];
				var param = {
					type : key
				};
	...
	$.fn.extend({
		...
		validate : function() {
			...
			if (arguments.length > 0 && typeof arguments[0] !== 'string') {
				...
				var namespace = target.namespace = target.type || PROFIX_NAMESPACE + $.getRandomString(4);
